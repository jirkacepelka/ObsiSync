...
A simple self-hosted sync server for [Obsidian](https://obsidian.md). No CouchDB, no S3 keys, no config files to tune.
...
1. **Install the plugin.** In the web admin click **Plugin → Download plugin (ZIP)**, unzip it into `<vault>/.obsidian/plugins/` and enable *Settings → Community plugins → ObsiSync*. On a phone, the easiest way is the **BRAT** plugin with the repository `jirkacepelka/ObsiSync`.
